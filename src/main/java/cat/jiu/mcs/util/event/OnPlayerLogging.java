...
public class OnPlayerLogging {
	...
	public static void onPlayerLogging(PlayerLoggedInEvent event) {
		...
		if(player.getName().indexOf(MCS.OWNER) == 0) {
			...
			if(JiuUtils.day.isTheDay(9, 7)) {
				if(date.getDay() == 7) {
					JiuUtils.entity.sendClientMessage(player, "Happy birthday! My Master!");
					
					player.addItemStackToInventory(new ItemStack(MCSItems.minecraft.food.C_ENCHANTED_GOLD_APPLE_F, 1, 9));
				}else {
					JiuUtils.entity.sendClientMessage(player, "\u4e3b\u4eba\u60a8\u6765\u5566\uff01\u8fd9\u662f\u7ed9\u60a8\u7684\u4ff8\u7984");
			...
			if(Configs.Custom.custem_already_stuff.logging_give_food) {
				...
				if(!(stack.getItem() instanceof ItemAir)) {
					JiuUtils.entity.sendClientMessage(player, "info.mcs.logging.name");
					
					JiuUtils.entity.sendServerMessage(player, stack.getDisplayName() + " x " + stack.getCount());
	...
	private static ItemStack randStack(EntityPlayer player) {
		...
		ItemStack stack = new ItemStack(MCSItems.FOODS.get(item), amount, meta);
		
		if(stack.getItem() instanceof ItemAir) {
			if(rand.nextInt(20) > 10) {
				JiuUtils.entity.sendClientMessage(player, I18n.format("info.mcs.logging.air.name", 0));
				return stack;
			}else {
