...
public class CraftCompressedStuffTrigger extends BaseAdvancement.BaseCriterionTrigger<CraftCompressedStuffTrigger> {
	...
	protected CraftCompressedStuffTrigger(ItemStack stack) {
		this(stack.getItem(), stack.getMetadata());
	}
	protected CraftCompressedStuffTrigger(Item item, int meta) {
		super(new ResourceLocation(MCS.MODID, "craft_compressed"));
		this.meta = meta > -1 ? meta : -1;
		this.item = item != null || item != Items.AIR ? item : null;
	}
	
	@Override
	public CraftCompressedStuffTrigger getInstance(JsonObject json, JsonDeserializationContext context) {
