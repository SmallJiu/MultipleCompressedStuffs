...
//	}
	...
	public int getHealAmount(ItemStack stack) {
		...
		if(this.baseItemIsNotFood) {
			if(Configs.Tooltip_Information.get_actual_food_amout) {
				...
				return (int)(this.healAmount + ((stack.getMetadata() + 1) * 0.9) * n);
			}
		}else {
			if(Configs.Tooltip_Information.get_actual_food_amout) {
				return (int) Math.pow(this.baseFood.getHealAmount(this.unCompressedItem), stack.getMetadata() + 1);
			}else {
				return (int)(this.baseFood.getHealAmount(this.unCompressedItem) + ((stack.getMetadata() + 1) * 0.9) * n);
			}
		}
	}
	...
	public float getSaturationModifier(ItemStack stack) {
		...
		if(this.baseItemIsNotFood) {
			if(Configs.Tooltip_Information.get_actual_food_amout) {
				...
				return (float)((double)this.saturationModifier + ((stack.getMetadata() + 1) * n));
			}
		}else {
			if(Configs.Tooltip_Information.get_actual_food_amout) {
				return (float)((double) Math.pow(this.baseFood.getSaturationModifier(this.unCompressedItem), stack.getMetadata() + 1));
			}else {
				return (float)((double)this.baseFood.getSaturationModifier(this.unCompressedItem) + ((stack.getMetadata() + 1) * n));
			}
		}
    }
	...
	public BaseItemFood addPotionEffect(int meta, PotionEffectType... effects) {
	...
	public BaseItemFood addPotionEffect(int[] metas, PotionEffectType[]... effects) {
		if(metas.length == 1) {
			if(metas[0] == -1) {
				List<PotionEffectType> effect = Lists.newArrayList(effects[0]);
				for(int i = 0; i < 16; i++) {
			...
			for(int i = 0; i < metas.length; i++) {
				int meta = metas[i];
				List<PotionEffectType> effect = Lists.newArrayList(effects[i]);
